...
const PostFX = React.lazy(() => import("./PostFX"));
...
function WireframeKnot() {
  ...
  return (
    <mesh ref={ref} position={[0, 0, -0.8]}>
      <torusKnotGeometry args={[2.8, 0.02, 160, 12]} />
      <meshBasicMaterial
        color={"#6D28D9"}
        wireframe
        transparent
...
function sampleTextToPoints(
  ...
  // High-fidelity sampler for legible text contours
  const w = options?.width ?? 1200;
  const h = options?.height ?? 320;
  const density = options?.density ?? 0.6; // higher => more points
  ...
  // Bold stroke + fill to thicken glyphs for sampling
  const base = Math.min(w, h);
  ctx.font = `900 ${Math.floor(base * 0.6)}px 'Inter', system-ui, -apple-system, sans-serif`;
  ctx.lineJoin = "round";
  ctx.lineCap = "round";
  ctx.lineWidth = Math.max(2, Math.floor(base * 0.14));
  ctx.strokeStyle = "#fff";
  ...
  for (let y = 0; y < h; y += step) {
    for (let x = 0; x < w; x += step) {
      ...
      if (alpha > 128) {
        ...
        const ny = (-(y - h / 2) / (h / 2)) * 2.6; // slightly taller for legibility
...
function ParticleMorph() {
  ...
  const [count, setCount] = useState(1400); // higher baseline for sharper text
  ...
  // One-time mobile tuning: adjust particle count for device
  useEffect(() => {
    try {
      const dm = (navigator as any)?.deviceMemory ?? 4;
      const w = window.innerWidth;
      const isSmall = w <= 640;
      const target = dm <= 3 || isSmall ? 1100 : 1600;
      setCount(target);
    } catch {}
  }, []);
  ...
  const textPositions = useMemo(() => {
    // dense sampling for legibility, then evenly distribute to match `count`
    const dense = sampleTextToPoints("ZEROMOTION", {
      width: 1400,
      height: 360,
      density: 0.7,
    });
    ...
    for (let i = 0; i < count; i++) {
      const idx = Math.min(
        total - 1,
        Math.floor((i / Math.max(1, count - 1)) * (total - 1)),
      );
      const p = dense[idx];
      arr.set([p[0], p[1], 0], i * 3);
    }
    return arr;
  }, [count]);
  ...
  useFrame((_s, d) => {
    ...
    const target =
      targetRef.current === "text" ? textPositions : cloudPositions;
    const pos = group.current.geometry.getAttribute(
      "position",
    ...
    // Orientation: face viewer in text mode, rotate only in cloud mode
  ...
  return (
    <>
      <Points ref={group} positions={positions} stride={3}>
        <PointMaterial
          size={toText ? 0.09 : 0.035}
          sizeAttenuation={!toText}
          transparent
          depthWrite={false}
          opacity={toText ? 1.0 : 0.78}
          color={"#A78BFA"}
        />
      </Points>
      ...
      <Html position={[0, 0, 0]} transform={false} zIndexRange={[20, 0]}>
        <div
          className="pointer-events-auto absolute right-3 bottom-3"
          style={{
...
const FallbackSVG: React.FC = () => (
  <div className="w-full h-full grid place-items-center bg-gradient-to-b from-zinc-900 to-zinc-950">
    <svg
      ...
      height="240"
      viewBox="0 0 240 240"
      className="drop-shadow-[0_0_35px_rgba(124,58,237,0.45)]"
    >
      <defs>
...
const QuantumShowcase: React.FC<Props> = ({
  className = "",
  intensity = 1,
}) => {
  ...
  return (
    <div
      ref={rootRef}
      className={`group relative rounded-2xl ring-1 ring-white/10 shadow-xl overflow-hidden ${className}`}
    >
      {showWebGL ? (
        <Canvas
          dpr={[1, 1.5]}
